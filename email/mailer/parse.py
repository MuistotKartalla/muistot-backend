...
class MailMap(UserDict[str, MailConfig]):
    ...
    def __getitem__(self, item: str) -> MailConfig:
        return super(MailMap, self).__getitem__(
            item.lower() if item is not None else item
        )

    def __contains__(self, item: str) -> bool:
        return super(MailMap, self).__contains__(
...
ENTRY = re.compile(
    r"(?P<lang>\w+(?:-\w+)?)\s*?=\s*?{(?P<value>(?:[^{}]|(?<=\\)[{}])+)}", re.MULTILINE
)
...
def parse(content: str) -> Mapping[str, MailConfig]:
    ...
    for m in ENTRY.finditer(content):
        ...
        out[lang] = MailConfig(
            sender=strip(_from), subject=strip(_subject), content=strip(_content)
        )
