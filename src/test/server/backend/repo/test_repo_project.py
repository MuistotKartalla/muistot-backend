...
async def pid(db):
    ...
    await db.execute(
        f"INSERT INTO projects (name, default_language_id, published) VALUE ('{name}', 1, 1)"
    )
    await db.execute(
...
async def test_delete_project(db, pid):
    ...
    assert await db.fetch_val(
        f"SELECT NOT EXISTS(SELECT 1 FROM projects WHERE name = '{pid}')"
    )
...
async def test_change_default_lang(db, pid, user):
    ...
    assert (
        await db.fetch_val(
            f"SELECT default_language_id FROM projects WHERE name = '{pid}'"
        )
        == 1
    )
...
async def test_change_start_end(db, pid, user):
    repo = ProjectRepo(db, *create_config(scopes.ADMIN, projects=[pid]))
    assert await db.fetch_val(
        f"SELECT (starts IS NULL AND ends IS NULL) FROM projects WHERE name = '{pid}'"
    )
    await repo.modify(
        pid,
        ModifiedProject(
    ...
    assert await db.fetch_val(
        f"SELECT (starts IS NOT NULL AND ends IS NOT NULL) FROM projects WHERE name = '{pid}'"
    )
...
async def test_contact_none_deletes(db, pid, user):
    ...
    assert await repo.modify(pid, ModifiedProject(contact=None))
    assert await db.fetch_val(
        """
...
async def test_localization_none_returns_false(db, pid, user):
    repo = ProjectRepo(db, *create_config(scopes.ADMIN, projects=[pid]))
    assert not await repo.modify(pid, ModifiedProject(info=None))
...
async def test_create_project_contact_none_ok(db, dpid, user):
    ...
    await repo.create(
        NewProject(
            id=dpid, contact=None, info=ProjectInfo(name="asdwadwadwdaw", lang="fi")
        )
    )
...
async def test_create_project_contact_ok(db, dpid, user):
    ...
    await repo.create(
        NewProject(
            ...
            info=ProjectInfo(name="asdwadwadwdaw", lang="fi"),
        )
    )
    assert await db.fetch_val(
        """
