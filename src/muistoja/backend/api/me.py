...
async def me(request: Request, db: Database = DEFAULT_DB):
    return UserData(**(await db.fetch_one(
        """
        SELECT u.username,
               u.email,
               upd.first_name,
...
async def log_me_out(request: Request):
    await request.state.manager.end_session(request.user.identity, request.user.token)


@router.delete('/me/sessions')
@require_auth(scopes.AUTHENTICATED)
async def log_me_out_all(request: Request):
    await request.state.manager.clear.sessions(request.user.identity)
