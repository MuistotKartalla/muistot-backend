...
class ProjectInfo(BaseModel):
    ...
    description: Optional[LONG_TEXT] = Field(
        description="Longer version of the description"
    )
...
class ProjectContact(BaseModel):
    ...
    contact_email: Optional[EmailStr] = Field(
        description="Contact email for the organizer"
    )
    has_research_permit: bool = Field(
        description="True if this project is running with a research permit"
    )
    can_contact: bool = Field(
        description="Does the project have permission to contact participants/researcher"
    )
...
class NewProject(BaseModel):
    ...
    admins: Optional[List[UID]] = Field(
        unique_items=True, description="Admins for the project"
    )
    contact: Optional[ProjectContact] = Field(
        description="Project contact details if any are available"
    )
    admin_posting: bool = Field(
        default=False, description="True if only admins can post"
    )
...
class ModifiedProject(BaseModel):
    ...
    info: Optional[ProjectInfo] = Field(
        description="Modifies the default locale for a project or sets a new one"
    )
    image: Optional[IMAGE] = Field(description=IMAGE_NEW)

    starts: Optional[datetime] = Field(
        description="Optional start date for the project"
    )
    ends: Optional[datetime] = Field(description="Optional end date for the project")

    contact: Optional[ProjectContact] = Field(
        description="Modified contact data if any"
    )
    ...
    class Config:
        __examples__ = {
            "lang": {
                ...
                "value": ProjectInfo.Config.__example__,
                "description": "Note that this changes the default language for the project. Use _localize_ instead.",
            }
        }
